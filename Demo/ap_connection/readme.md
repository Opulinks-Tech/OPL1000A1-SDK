# 包含文件说明(V1.0)
1. **ap_connection.exe**  : Opulinks 的 wpa2_station 工程编辑，编译，和下载工具, 旨在为用户提供一款便捷的低功耗测量工具。
2. **test_ap_connection.ini**  : Opulinks ap_connection 工具配置文件


# 修改配置文件test_ap_connection.ini参数
...
6. **uart_port 参数**：               用于下载m3.bin的真实有效的AT串口. 例如，COM12
...
# 使用指南：
**ap_connection.exe提供的功能如下**：

1. 读取并显示当前目录下的test_ap_connection.ini 文件，等待用户的输入以确定是否需要更新配置；

2. 若用户选择更新上述8个参数，终端将会依次输出文本提示用户输入具体内容。对于每个参数，用户可直接回车表示不更新该参数，并且最多有两次输入该参数的机会，若两次输入值均不被接受，将会不更新该参数并继续；
3. 若上述参数有更新，所有的更新将会写回test_ap_connection.ini 文件。之后将更新文件，编译工程，和生成相应的m3.bin文件；
4. 在最后下载m3.bin文件阶段，需要用户配合按复位按钮。请在终端输出提示信息之后的5秒内按复位键。若错过5秒的时间点，本次下载失败；然后显示是否再试一次的提示信息；
