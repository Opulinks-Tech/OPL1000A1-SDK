 ...
 */
#define DBG_UART_TIMEOUT_COUNT_MAX  ( 0x30000 )
...
uint32_t Hal_DbgUart_BaudRateSet_impl(uint32_t u32Baud)
{
    ...
    result = (SYSTEM_CLK + u32Baud/2)/u32Baud;
    if( (result<DBG_UART_BAUD_DIV_MIN) ||(result>DBG_UART_BAUD_DIV_MAX) )
    {
        DBG_UART->BAUD_DIV = (SYSTEM_CLK + DEFAULT_BAUD/2) / DEFAULT_BAUD;
        return 1;
    }else{
...
uint32_t Hal_DbgUart_Init_impl(uint32_t u32Baud)
{
    ...
    // Reset dbg_uart
    Hal_Sys_ApsModuleRst(ASP_RST_DBG_UART);
