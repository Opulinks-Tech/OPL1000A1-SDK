...
//typedef int (*wpa_drv_set_key_fp_t)(struct wpa_supplicant *wpa_s, enum wpa_alg alg, const u8 *addr, int key_idx, int set_tx, const u8 *seq, size_t seq_len, const u8 *key, size_t key_len);
...
//extern wpa_drv_set_key_fp_t wpa_drv_set_key;
...
#if 0
void * wpa_driver_netlink_init(void *ctx, const char *ifname);
void wpa_driver_netlink_deinit(void *priv);
Boolean wpa_driver_netlink_scan(int mode);
Boolean wpa_driver_netlink_get_scan_results(struct wpa_scan_results * res);
Boolean wpa_driver_netlink_connect(struct wpa_config * wconf);
Boolean wpa_driver_netlink_reconnect(struct wpa_config * conf);
Boolean wpa_driver_netlink_associate(void *priv, struct wpa_driver_associate_params *params);
void wpa_driver_netlink_disconnect(const u8 *bssid, u16 reason_code);
Boolean wpa_driver_netlink_deauthenticate(void *priv, const u8 *addr, int reason_code);
Boolean wpa_driver_netlink_get_bssid(u8 *bssid);
Boolean wpa_driver_netlink_get_ssid(u8 *ssid);
Boolean wpa_driver_netlink_get_mac(u8 *mac);
int wpa_driver_netlink_set_key(const char *ifname, void *priv, enum wpa_alg alg, const u8 *addr, int key_idx, int set_tx, const u8 *seq, size_t seq_len, const u8 *key, size_t key_len);
void wpa_driver_netlink_scan_results_free(struct wpa_scan_results *res);
int wpa_drv_set_key(struct wpa_supplicant *wpa_s, enum wpa_alg alg, const u8 *addr, int key_idx, int set_tx, const u8 *seq, size_t seq_len, const u8 *key, size_t key_len);
int wpa_drv_send_eapol(const u8 *dst, u16 proto, u8 *data, size_t data_len);
Boolean wpa_driver_netlink_connect_by_bssid(u8 *bssid);
void wpa_driver_netlink_set_mac(u8 *mac);
#endif
