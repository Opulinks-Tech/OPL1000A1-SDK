			       ...
			       struct wpa_ptk *ptk)
{
    ...
    for(i=0;i<wpa_ie_len;i++){
        wpa_printf(MSG_WARNING, "%02x ", wpa_ie[i]);
    }
    ...
    */
    ...
    for(i=0;i<16;i++) wpa_printf(MSG_ERROR, "%02x ", reply->key_mic[i]);
    wpa_printf(MSG_DEBUG, "\r\n");
...
//typedef int (*wpa_supplicant_validate_ie_fp_t)(struct wpa_sm *sm, unsigned char *src_addr, struct wpa_eapol_ie_parse *ie)
    ...
    for(i=0; i<16; i++) wpa_printf(MSG_WARNING, "%02x ", sm->ptk.kck[i]);
    wpa_printf(MSG_DEBUG, "\r\n");

    wpa_printf(MSG_DEBUG, "[3_4]WPA: sm->ptk.kek:");
    for(i=0; i<16; i++) wpa_printf(MSG_WARNING, "%02x ", sm->ptk.kek[i]);
    wpa_printf(MSG_DEBUG, "\r\n");
    ...
    for(i=0; i<ie.gtk_len; i++) wpa_printf(MSG_WARNING, "%02x ", *(ie.gtk + i));
    wpa_printf(MSG_DEBUG, "\r\n");
...
//    int ret, i;
    ...
    wpa_printf(MSG_WARNING, "[2_2][GTK]WPA: reply->key_info:%x %x \r\n", reply->key_info[0], reply->key_info[1]);
    ...
    */
    ...
    //wpa_printf(MSG_DEBUG, "\r\n [GTK] wpa_supplicant_send_2_of_2, ret:%d reply->key_mic[]:", ret);
    //for(i=0; i<16; i++) wpa_printf(MSG_DEBUG, "%02x ", reply->key_mic[i]);
		    ...
		    const u8 *buf, size_t len)
{
    ...
    wpa_printf(MSG_WARNING, "[KEY]WPA: sm->rx_replay_counter_set:%d \r\n", sm->rx_replay_counter_set); //sm->rx_replay_counter_set:0
    wpa_printf(MSG_WARNING, "[KEY]WPA: key->replay_counter[0~7]: %x %x %x %x %x %x %x %x \r\n", //key->replay_counter[0~7]: 0 0 0 0 0 0 0 1
    ...
    wpa_printf(MSG_WARNING, "[KEY]WPA: sm->rx_replay_counter[0~7]: %x %x %x %x %x %x %x %x \r\n", //sm->rx_replay_counter[0~7]: 0 0 0 0 0 0 0 0
...
//need to check here in the future
    ...
    wpa_printf(MSG_WARNING, "[KEY]WPA: key_info:%x \r\n", key_info)
