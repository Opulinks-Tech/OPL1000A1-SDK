...
#define BLE_DEVICE_NAME					"BLEWIFI APP"
...
static UINT16 gBleWifiSvcUuid = 0xAAAA;

static UINT16 gBleWifiDataInUuid          = 0xBBB0;
static UINT8  gBleWifiDataInCharVal[]     = CHAR_DECL_UUID16_ATTR_VAL(LE_GATT_CHAR_PROP_WR_NO_RESP | LE_GATT_CHAR_PROP_WR, 0xBBB0);
static UINT8  gBleWifiDataInVal[LE_ATT_MAX_MTU];

static UINT16 gBleWifiDataOutUuid		  = 0xBBB1;
static UINT8  gBleWifiDataOutCharVal[]    = CHAR_DECL_UUID16_ATTR_VAL(LE_GATT_CHAR_PROP_NTF, 0xBBB1);
...
static void BleAppHandleBwpServiceRead(LE_GATT_MSG_ACCESS_READ_IND_T *ind)
{
	UINT8 attErr = 0;
	...
	switch (attrid)
    {
		case BWP_IDX_DATA_OUT_CFG:
        {
			...
			BLE_APP_PRINT("BWP_IDX_DATA_OUT_CFG val = %d\r\n", enable);
        }
		break;
...
static void BleAppHandleBwpServiceWrite(LE_GATT_MSG_ACCESS_WRITE_IND_T *ind)
{
	UINT8 attErr = 0;
	UINT16 attrid = ind->handle - gBwpSvc->startHdl;
    BLE_APP_PRINT("APP - BleAppHandleBwpServiceWrite attId = %d op = %x offset = %d len = %d\r\n", attrid, ind->flag, ind->offset, ind->len);
	switch (attrid)
    {
		case BWP_IDX_DATA_IN_VAL:
		{
			// Send to WIFI task
			//APP_MEMDUMP(ind->pVal, ind->len);

            blewifi_ctrl_msg_send(BLEWIFI_CTRL_MSG_BLE_DATA_IND, ind->pVal, ind->len);
        }
		break;
...
static void BleGattHandleAccessRead(LE_GATT_MSG_ACCESS_READ_IND_T *ind)
{
	...
	{
		BleAppHandleBwpServiceRead(ind);
    }
	else
	{
...
static void BleGattHandleAccessWrite(LE_GATT_MSG_ACCESS_WRITE_IND_T *ind)
{
	...
	{
		BleAppHandleBwpServiceWrite(ind);
    }
	else
	{
...
void BleWifiServerAppGattMsgHandler(TASK task, MESSAGEID id, MESSAGE message)
{
	switch (id)
	{
		case LE_GATT_MSG_NOTIFY_CFM:
        {
			BLEWIFI_DATA_OUT_STORE_T *s = &BleAppGetEntity()->store;

			if (s->sending) s->sending--;

            if (s->pidx != s->ridx) LeSendMessage(&BleAppGetEntity()->task, BLEWIFI_APP_MSG_SEND_TO_PEER, 0);
        }
        break;
		...
		case LE_GATT_MSG_INIT_CFM:
		{
			...
			if (gBwpSvc)
			{
				BLE_APP_PRINT("LeGattRegisterService gBwpSvc success\r\n");
            }

            BleAppGetEntity()->store.send_hdl = gBwpSvcDb[BWP_IDX_DATA_OUT_VAL].handle;
        }
		break;
...
void BleWifiGattIndicateServiceChange(UINT16 conn_hdl)
{
	UINT16 len;
	UINT16 val;
