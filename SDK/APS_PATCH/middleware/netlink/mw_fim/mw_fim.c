...
T_MwFim_FlashLayoutUpdate_Fp MwFim_FlashLayoutUpdate = NULL;


// Sec 5: declaration of global function prototype
void MwFim_Init(void);
uint8_t MwFim_FileRead(uint32_t ulFileId, uint16_t uwRecIdx, uint16_t uwFileSize, uint8_t *pubFileData);
uint8_t MwFim_FileWrite(uint32_t ulFileId, uint16_t uwRecIdx, uint16_t uwFileSize, uint8_t *pubFileData);
uint8_t MwFim_FileWriteDefault(uint32_t ulFileId, uint16_t uwRecIdx);
uint8_t MwFim_FileDelete(uint32_t ulFileId, uint16_t uwRecIdx);

uint8_t MwFim_ZoneInfoUpdate(uint32_t ulZoneIdx, T_MwFimZoneInfo *ptZoneInfo);
uint8_t MwFim_GroupInfoUpdate(uint32_t ulZoneIdx, uint32_t ulGroupIdx, T_MwFimFileInfo *ptGroupInfo);
uint8_t MwFim_GroupVersionUpdate(uint32_t ulZoneIdx, uint32_t ulGroupIdx, uint8_t ubVersion);
...
static uint8_t g_ubMwFimInit;
static osSemaphoreId g_tMwFimSemaphoreId;
static T_MwFimGroupStatus g_taMwFimGroupStatusTable[MW_FIM_ZONE_MAX][MW_FIM_GROUP_MAX];


// Sec 7: declaration of static function prototype
static uint8_t MwFim_FileWriteDo(uint32_t ulFileId, uint16_t uwRecIdx, uint16_t uwFileSize, uint8_t *pubFileData);
static void MwFim_GroupSizeCheck(void);
static void MwFim_GroupStatusFill(void);
static void MwFim_GroupHeaderCheck(uint32_t ulZoneIdx, uint8_t ubaBlockUsed[MW_FIM_GROUP_MAX]);
static uint8_t MwFim_GroupHeaderVerify(uint32_t ulZoneIdx, T_MwFimGroupHeader *ptGroupHeader);
static uint8_t MwFim_GroupCheckSumVerify(T_MwFimGroupHeader *ptGroupHeader);
static void MwFim_GroupStatusRemap(uint32_t ulZoneIdx, uint8_t ubaBlockUsed[MW_FIM_GROUP_MAX]);
static uint8_t MwFim_EmptyBlockFind(uint32_t ulZoneIdx, uint8_t ubaBlockUsed[MW_FIM_GROUP_MAX], uint32_t *pulIndex);
static void MwFim_GroupBlockParser(void);
static uint32_t MwFim_FileHeaderVerify(T_MwFimFileInfo *ptFileTable, T_MwFimFileHeader *ptFileHeader);
static uint32_t MwFim_FreeOffsetVerify(uint32_t ulZoneIdx, uint32_t ulGroupIdx, uint32_t ulDataOffset);
static uint8_t MwFim_FileTableSearch(uint32_t ulFileId, T_MwFimFileInfo **pptFileTable);
static uint8_t MwFim_GroupSwap(uint32_t ulZoneIdx, uint32_t ulGroupIdx);
static void MwFim_FileDataDefaultFill(void);
