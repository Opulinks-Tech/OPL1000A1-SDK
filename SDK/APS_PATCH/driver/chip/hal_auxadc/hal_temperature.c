...
*  Project:
*  --------
*  NL1000 Project - the Temperature Sensor implement file
*
*  Description:
*  ------------
...
T_HalTmprCalData g_tHalTmpr_CalData;


// Sec 5: declaration of global function prototype
void Hal_Tmpr_Init(void);
uint8_t Hal_Tmpr_TemperatureGet(uint8_t ubGpioIdx, float *pfTemperature);
...
*
*************************************************************************/
uint8_t Hal_Tmpr_CompareResistor(float fResistor, float *pfTemperature)
{
...
*
*************************************************************************/
void Hal_Tmpr_CmdParse(char *sCmd)
{
    float fValue;
    uint32_t ulValue;
    uint8_t ubIdx;
    ...
    // get the first token
    pucaToken[0] = (uint8_t*)strtok(sCmd, (const char*)ucaSeps);
    ...
    if (NULL != pucaToken[0])
    {
        for (i=1; i<8; i++)
        {
            pucaToken[i] = (uint8_t*)strtok(NULL, (const char*)ucaSeps);
            if (NULL == pucaToken[i])
                break;
        }
    }
